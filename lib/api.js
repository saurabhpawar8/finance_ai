...
export const getTransactions = async (params = {}) => {
  ...
  if (params.dateFrom) query.set("date__gte", params.dateFrom);
  if (params.dateTo) query.set("date__lte", params.dateTo);
...
export const getMonthlyTotals = async () => {
  const res = await fetch(`${BASE}/api/transaction/monthly_totals`, {
    headers: authHeaders(),
  });
  if (check(res)) return;
  return res.json();
...
export const getBudget = async (id) => {
  const res = await fetch(`${BASE}/api/budget/${id}/`, {
    headers: authHeaders(),
  });
  if (check(res)) return;
  return res.json();
...
export const createBudget = async (category, budget_amount) => {
  const res = await fetch(`${BASE}/api/budget/`, {
    method: "POST",
    headers: authHeaders(),
    body: JSON.stringify({ category, budget_amount }),
  });
  if (check(res)) return;
  return res.json();
};

export const updateBudget = async (id, budget_amount) => {
  const res = await fetch(`${BASE}/api/budget/${id}/`, {
    method: "PUT",
    headers: authHeaders(),
...
export const loginWithGoogle = async (id_token) => {
  const res = await fetch(`${BASE}/account/auth/google/`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
