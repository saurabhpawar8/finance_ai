...
export const getSummary = async () => {
  const res = await fetch(`${BASE}/api/transaction/summary`, {
    headers: authHeaders(),
  });
  if (check(res)) return;
  return res.json();
...
export const getPieSummary = async () => {
  const res = await fetch(`${BASE}/api/transaction/pie_summary`, {
    headers: authHeaders(),
  });
  if (check(res)) return;
  return res.json();
...
export const getHeatmap = async () => {
  const res = await fetch(`${BASE}/api/transaction/heatmap`, {
    headers: authHeaders(),
  });
  if (check(res)) return;
  return res.json();
