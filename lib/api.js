...
const handle401 = () => { removeTokens(); window.location.href = '/auth'; };
...
const check = (res) => { if (res.status === 401) { handle401(); return true; } return false; };
...
export const getSummary = async (month, year) => {
  ...
  const res = await fetch(`${BASE}/api/transaction/summary${q}`, { headers: authHeaders() });
  if (check(res)) return; return res.json();
};

export const getPieSummary = async (month, year) => {
  const q = month && year ? `?month=${month}&year=${year}` : '';
  const res = await fetch(`${BASE}/api/transaction/pie_summary${q}`, { headers: authHeaders() });
  if (check(res)) return; return res.json();
};

export const getHeatmap = async (month, year) => {
  const q = month && year ? `?month=${month}&year=${year}` : '';
  const res = await fetch(`${BASE}/api/transaction/heatmap${q}`, { headers: authHeaders() });
  if (check(res)) return; return res.json();
};
...
export const getTransactions = async (params = {}) => {
  ...
  const res = await fetch(`${BASE}/api/transaction?${query}`, { headers: authHeaders() });
  if (check(res)) return; return res.json();
};
...
export const getCategories = async () => {
  const res = await fetch(`${BASE}/api/category/category_list`, { headers: authHeaders() });
  if (check(res)) return; return res.json();
};

export const getWallets = async () => {
  const res = await fetch(`${BASE}/api/wallet/wallet_list`, { headers: authHeaders() });
  if (check(res)) return; return res.json();
};
